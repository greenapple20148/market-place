...
const ManageInventory: React.FC<ManageInventoryProps> = ({ user }) => {
  ...
  const fetchProducts = async () => {
    setIsLoading(true);
    try {
      const { products: data, total } = await productsService.getSellerProducts(user.id, currentPage, PAGE_SIZE);
      setProducts(data);
      setTotalCount(total);
    } catch (err) {
      console.error(err);
                      ...
                      <tr key={p.id} className="group hover:bg-stone-50/50 dark:hover:bg-stone-800/20 transition-colors">
                        ...
                        <td className="px-6 py-5">
                          <div className="flex justify-end gap-2">
                            <Link to={`/seller/products/edit/${p.id}`} className="p-2 text-stone-400 hover:text-brand-600 transition-colors"><Edit3 className="w-4 h-4" /></Link>
                            <button onClick={() => handleDelete(p.id)} className="p-2 text-stone-400 hover:text-red-500 transition-colors"><Trash2 className="w-4 h-4" /></button>
                          </div>
                        </td>
                      </tr>
                    ))}
